...
function retornaFeriadosItajai($data=''){
	...
	// Conecta na API
    $curl = curl_init('https://api.calendario.com.br/?json=true&token=cGVkcm9AbWVjb2IuY29tLmJyJmhhc2g9NTc1OTgzNzc&ano='.$data[0].'&ibge=4208203');
...
function protocolos_feriados_range($startDate=null, $endDate=null) {
	...
	if($startDate == null ) {
		...
		// $startDate = date('Y-m-d', strtotime('2019-11-08'));
	...
	// syslog( 158, 'MECOB - ' . date('H:i:s') . ' - Dia swap 0 ' . $startDate . " " . $endDate);
	// syslog( 158, 'MECOB - ' . date('H:i:s') . ' - Dia swap 1 ' . json_encode($holidays) . " " . $dias_uteis);	syslog( 158, 'MECOB - ' . date('H:i:s') . ' - Dia swap 1 ' . json_encode($swap_date) . " " . $dias_uteis);

	// Retorna a data final somados os feriados na semana e os finais de semana.
	$swap_date = getDateUtil($swap_date, $holidays);
	// syslog( 158, 'MECOB - ' . date('H:i:s') . ' - Dia swap 2 ' . json_encode($swap_date));
	return $swap_date;
}

function getDateUtil($asuumed_date, $holidays) {
    if (in_array(date('d/m/Y', strtotime($asuumed_date)), $holidays)) {
