...
foreach($parcelas as $parcela){
	...
	if($nu_parcela <= 30) {
		$content_all .= $content;
	} else if($nu_parcela <= 60) {
		$content_all2 .= $content;
	} else if($nu_parcela <= 90) {
		$content_all3 .= $content;
	} else {
		$content_all4 .= $content;
	}
...
try
{
	...
	$html2pdf->writeHTML($content_all2, isset($_GET['vuehtml']));
	$html2pdf->writeHTML($content_all3, isset($_GET['vuehtml']));
	$html2pdf->writeHTML($content_all4, isset($_GET['vuehtml']));
	
	/* Abrir no navegador */
	$html2pdf->Output($novo_arquivo);
	// $html2pdf->Output($novo_arquivo, 'D');
