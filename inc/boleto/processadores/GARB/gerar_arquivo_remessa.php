...
function gerar_arquivo_remessa($contrato_id, $conexao_BD_1, $tipo = "", $parcelas_id = "", $dados_parcela = []){
    ...
    // } else {
        ...
        if (count($parcelas)) {
            ...
            $nm_ini =  "CB" . date('dmy')."_".$milissegundos;
            ...
            $config['razao_social'] = 'UNICRED FLORIANOPOLIS';
            ...
            foreach ($parcelas as $parcela) {
                ...
                $bairro_pagador = $funcoes->montar_branco($funcoes->remover_acentos(substr($parcela["bairro"],0,20)),20, 'right');
                $cidade_pagador = $funcoes->montar_branco($funcoes->remover_acentos(substr($parcela["cidade"],0,38)),38, 'right');
