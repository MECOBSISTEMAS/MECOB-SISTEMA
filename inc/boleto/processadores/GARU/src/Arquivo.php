...
class Arquivo {
	...
	public function add_boleto($boleto) {
		...
		$detalhes->setEstado_pagador($boleto['estado_pagador']);
		...
		// Indentifica a ocorrencia (Remessa, cancelamento, update), default 01 - Remessa
		$detalhes->setIdentificacao_ocorrencia($boleto['identificacao_ocorrencia']);
	...
	public function get_text() {
		...
		foreach ($this->getDetalhes() as $detalhe) {
			...
			// Imprime o regisro tipo 7 
			// $detalhe->setNumero_sequencial_registro($numero_sequencial++);
			// $dados .= $detalhe->montar_linha_tipo7() . self::QUEBRA_LINHA;
		}
		//montando rodap�
	...
	public function inserir(&$conexao_BD_1, $nm_arq, $dt_arq, $tp_arq, $contrato_id = null, $origem, $boletos_avulso_id = null ){

	    $status = "AGUARDANDO_ENVIO";

		$insert_arq = "insert into arquivos 
								( nm_arq, dt_arq, tp_arq, contratos_id, status, origem, boletos_avulso_id  )
						values  ( '".$nm_arq."', '".$dt_arq."', '".$tp_arq."' ,".$contrato_id.", '".$status."', '".$origem."', ".$boletos_avulso_id.") ";
		return $conexao_BD_1->query_inserir($insert_arq);
