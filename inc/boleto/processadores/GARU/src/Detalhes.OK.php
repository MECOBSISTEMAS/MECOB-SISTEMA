...
class Detalhes extends Funcoes implements IFuncoes {
	...
	//105 - 105 - 1 - A Tipo de mora (diária) 
	private $tipo_mora = '1';
	...
	public function setPercentual_multa($percentual_multa) {
		...
		if($this->getCampo_multa()) {
			...
			$this->percentual_multa = $this->montar_branco('000', 10, 'right');
		}
	}
	 ...
	 */
	public function setBairo_pagador($bairro_pagador) {
		//adiciona brancos na string
	...
	public function montar_linha() {
		...
		$linha = 
			...
			// $this->getEspecie_tituloEspecie_titulo() .  //nao seta
			...
			// syslog( 158, 'MECOB - ' . date('H:i:s') . ' - Arquivos Remessa Detalhes 136 -- ' . $this->numero_unicred_compensacao);
