...
function gerar_arquivo_remessa($contrato_id, $conexao_BD_1, $tipo = "", $parcelas_id = "", $dados_parcela = []){
    ...
    if ($tipo === "BOLETO_AVULSO") {
        ...
        $contrato_id = "null";
        $parcelas = $boletos_avulsos_funcoes_DB->dados_boleto_avulso_cancelar($conexao_BD_1, $boletos_avulso_id);
    } else {
        $boletos_avulso_id = "null";
        $parcelas = $contratos_funcoes_DB->dados_boleto_cancelar($conexao_BD_1, $contrato_id);
    }
    ...
    // } else {
        ...
        if (count($parcelas)) {
            ...
            foreach ($parcelas as $parcela) {
                ...
                $boleto['data_emissao_titulo'] = date('dmy');
                $boleto['valor_dia_atraso'] = '0033';
                ...
                $boleto['identificacao_ocorrencia'] = '02';
                // syslog( 158, 'MECOB - ' . date('H:i:s') . ' - Arquivos Remessa ' . $boleto['identificacao_ocorrencia']);
