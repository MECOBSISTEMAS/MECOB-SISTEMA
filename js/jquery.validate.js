...
$.extend($.validator, {
	...
	messages: {
		...
		maxlength: $.format("Tamanho máximo: {0} caracteres."),
		minlength: $.format("Tamanho mínimo: {0} caracteres."),
	...
	methods: {
		...
		diferente: function(value, element, param) {
			return value != param;
		}
...
jQuery.validator.addMethod("dateBR", function(value, element) {
	 if(value=='__/__/____') return false;
	...
	var anoMinimo = dataHoje.getFullYear();
	var anoMaximo = dataHoje.getFullYear();
	anoMinimo -= 10; //O usuário deve ter no mínimo 9 anos.
	anoMaximo -= 100; //O usuário deve ter no máximo 101 anos.
...
jQuery.validator.addMethod("numeroImovel", function(value, element) {	 
	if(value <= 0 )return false;
	return true;
}, "O número não pode ser menor ou igual a zero");


jQuery.validator.addMethod("foneValido", function(value, element) {	 
	var ddd = value.substr(1,2);
	ddd = parseInt(ddd);
	if( ddd < 10 ) return false;
	
	var numb4 = value.substr(5,4);
	numb4 = parseInt(numb4);
	if(numb4 < 1000 )return false;
...
}, jQuery.format("Entre com o Nome completo"));

jQuery.validator.addMethod("completo", function(value, element) { 
var quebra = value.split(" ");
if(quebra[0]==" " || quebra[0].length < 2 || quebra[1]==" " || quebra[1].length < 1)  return false;
else return true; 
}, jQuery.format("Entre com o nome completo"));
