...
switch ($_REQUEST["acao"]) {
	...
	case 'listaContratosVendedor':
		...
		$ordem = $_REQUEST['ordem'];
		// syslog( 158, 'MECOB - ' . date('H:i:s') . ' - Contratos Vendedor Filtros ' . json_encode($filtros));
		$retorno = $contratosDB->lista_contratos($ct, $conexao_BD_1,  $filtros ,  $ordem ,  0,"N");
	break;
...
if($envia_email){
	...
	$emailDB  = new emailDB();
	$emailDB->insert_send_mail($conexao_BD_1, $destinatarios, $assunto, $mensagem, "", "", "olavo@mecob.com.br", "MECOB",9);
}
