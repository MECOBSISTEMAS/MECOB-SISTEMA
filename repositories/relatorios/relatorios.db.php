...
class relatoriosDB{
    ...
    function lista_saldo_cliente(&$conexao_BD_1, $datai=null, $dataf=null, $filtro_status=null) {

        // syslog( 158, 'MECOB - ' . date('H:i:s') . ' - Filtros Status ' . json_encode($filtro_status));
        ...
        $query = "
            ...
            WHERE 1=1
                    AND c.status IN ('".implode("','", $filtro_status)."')
    ...
    function lista_boletos_adimplencia(&$conexao_BD_1, $datai=null) {
        ...
        SELECT 
            ...
            count(*) as total,
            sum(case when cp.vl_pagto > 0 then cp.vl_pagto else cp.vl_parcela end) as vl_total,

            sum(case when (cp.vl_pagto > 0 or (not cp.dt_pagto = '0000-00-00' and not isnull(cp.dt_pagto))) then 1 else 0 end) as pago,
            sum(case when (cp.vl_pagto > 0 or (not cp.dt_pagto = '0000-00-00' and not isnull(cp.dt_pagto))) then if(cp.vl_pagto > 0, cp.vl_pagto, cp.vl_parcela) else 0 end) as vl_pago,

            sum(case when (vl_pagto > 0 or (not cp.dt_pagto = '0000-00-00' and not isnull(cp.dt_pagto))) then 0 else 1 end) as n_pago,
            sum(case when (vl_pagto > 0 or (not cp.dt_pagto = '0000-00-00' and not isnull(cp.dt_pagto))) then 0 else cp.vl_parcela end) as vl_n_pago,
