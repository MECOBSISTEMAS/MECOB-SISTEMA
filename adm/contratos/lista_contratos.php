    ...
    <div class="modal-dialog wd-90p" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <?php if (in_array($_SESSION['perfil_id'], array('1', '3'))) {?>

                    <button id='cancelar_boletos' type="button" class="btn btn-red"
                    onclick="cancelar_boletos();return false;">
                        Cancelar boletos
                    </button>

                    <!-- <button id='atualizar_boletos' type="button" class="btn btn-red"
...
function alimenta_modal_cad_contratos(id, contratos, tipo, copia) {
    ...
    if (id == 0) {
        ...
        // Permite troca de vendedor pelo Admin
            ...
            for (var i = 0; i < j.length; i++) {
                ...
                if (j[i].status != 'excluido'){
                    ...
                    if (((jQuery.inArray(<?php echo $_SESSION['perfil_id']; ?>, [1, 3] ) >= 0) || "<?php echo $_SESSION['supervisor']; ?>" == "S") && (j[i].status == "confirmado" || j[i].status == "pendente")) {
                        ...
                        } else if (((jQuery.inArray(<?php echo $_SESSION['perfil_id']; ?>, [1, 3] ) >= 0) || "<?php echo $_SESSION['id']; ?>" == "2695999999999999") && (j[i].status == "confirmado" || j[i].status == "pendente")) {
                    ...
                    if(j[i].status == 'confirmado') {
                        <?php if(in_array($ck_mksist_permissao['perfil_id'], array('1', '3'))) { ?>
...
$("#listagem_contratos").on('click', '.suspender_contrato', function() {
    ...
    jConfirm(`Suspendendo o contrato, todas as cobranças referente a este serão suspensas, até o mesmo ser reativado.
        ...
        if (r) {
            if ($('.suspender_contrato').data('supervisor') == 'S' && "<?php echo $_SESSION['id']; ?>" != "2695999999999999"){
...
function alimenta_modal_ocorrencias(contrato_id) {
    ...
    $.getJSON(
        '<?php echo $link . "/repositories/ocorrencias/ocorrencias.ctrl.php?acao=lista_ocorrencias_id"; ?>&inicial=' +
        exibidos, {
            contrato_id: contrato_id,
            ajax: 'true'
        },
        function(j) {
            ...
            for (var i = 0; i < ocorrencias.length; i++) {
                ...
                if (ocorrencias[i].o_status == 'Cliente') {
                    html_ocorrencias += "<div class='ocor_msg' style='background:#fff675' >";
                } else {
...
?>
    ...
    function cancelar_boletos() {
        var contrato_id = $("input#inputId").val();

        jConfirm('Será gerado um arquivo de remessa com o cancelamento e o mesmo será enviado ao banco.',
                'Deseja realmente cancelar os boletos do contrato ID '+contrato_id+' na UNICRED?',
            function(r) {
                if (r) { 
                    ...
                    if( result == 1 ){
                        jAlert(result.msg,'Gerado arquivo de cancelamente!','ok');
                    }
                    else{
